...
function echoOf(trail?: ReflectionPoint[]) {
  ...
  const latest = trail[trail.length - 1]?.at ?? 0;
  return trail.find(p => p.at >= latest - ARENA.delayMs);
}
...
class MazeScene extends Phaser.Scene {
  ...
  create() { this.ready = true; this.draw(); }
  private draw() {
    ...
    Object.values(this.snapshot.runners).forEach(runner => {
      const player = this.players[runner.id]; const echo = echoOf(runner.trail);
