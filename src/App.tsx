...
import { createRoom, joinRoom, resetRoom, sendInput, startRoom, watchRoom, writeSnapshot } from './services/rooms';
...
export default function App() {
  ...
  const [name, setName] = useState(''); const [roomCode, setRoomCode] = useState(inviteRoom); const [uid, setUid] = useState(''); const [room, setRoom] = useState<Room | null>(null); const [error, setError] = useState(''); const [now, setNow] = useState(serverNow()); const [roomMissing, setRoomMissing] = useState(false); const [copied, setCopied] = useState(false); const input = useRef<InputIntent>(initialInput);
  const roomRef = useRef<Room | null>(null);
  useEffect(() => { roomRef.current = room; }, [room]);

  const move = useCallback((x: number, y: number) => { input.current = { ...input.current, x, y, updatedAt: Date.now() }; }, []);
  const dash = useCallback(() => { input.current = { ...input.current, dash: true, updatedAt: Date.now() }; setTimeout(() => input.current = { ...input.current, dash: false, updatedAt: Date.now() }, 200); }, []);
  ...
  const copyInvite = async () => {
    try {
      ...
      window.prompt('Copy invite link:', location.href);
    }
  };
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (done || now - lastWrite >= 100) {
        lastWrite = now;
        writeSnapshot(code, state, done ? 'results' : undefined).catch(console.error);
      }
